...
// required timeframe
func (c *Client) Quotes(baseCurrency string, quoteCurrency string, fromTimestamp uint64, toTimestamp uint64) (quotes []*PriceQuote, err error) {

	symbol := currencyPairSymbol(baseCurrency, quoteCurrency)

	c.baseURL.Path = path.Join("v8/finance/chart", symbol)
	c.baseURL.RawQuery = quotesQuery(symbol, fromTimestamp, toTimestamp)

	response := &quotesResponseContent{}
